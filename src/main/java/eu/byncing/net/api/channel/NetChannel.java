...
public class NetChannel implements IChannel, Runnable {
    ...
    public NetChannel(INetStructure structure, Socket socket) {
        ...
        while (connected) {
            try {
                ...
                while (connected && (read = inputStream.readByte()) > -1) {
                    count++;
                    if (structure.getOption(NetOption.BUFFER_SIZE) < count) {
                        throw new Exception("The buffer is too big, allowed size " + structure.getOption(NetOption.BUFFER_SIZE));
                    }
                    array.write(read);
                }
